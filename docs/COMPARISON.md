...
**P0（新增 — 第一性原理）：**
5. ✅ 添加 `skills/self-improvement/` — 每次冲刺结束时的第一性原理自我评估技能
6. ✅ 在 `lib/preamble.md` 中添加 Performance Checkpoint — 每个技能的完成状态自检注入所有 28 个技能

**P1（竞争力差距）：**
7. 团队共享机制（`setup --team` 命令，将 superomni 提交到项目仓库）
8. 自动升级机制（`bin/superomni-cli upgrade` 命令）
9. 技能间数据传递约定（定义技能输出格式，供下游技能读取）
