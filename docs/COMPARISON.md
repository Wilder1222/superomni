...
**版本：** superomni v0.6.10 / superpowers latest / gstack latest  
**生成日期：** 2026-03-27  
**作者：** superomni 项目分析
...
**背景：** superomni 明确将自己定位为"融合体"——从 superpowers 吸收方法论驱动的工作流，从 gstack 吸收工程完整性与决策原则，同时加入原创的状态协议（Status Protocol）和多平台支持。
...
| **技能触发方式** | 自动触发（条件检测，强制工作流） | 手动调用斜杠命令 + 被动建议 | 强制触发（/vibe 命令触发完整流水线） |
...
```
- **优势**：`{{PREAMBLE}}` 宏确保所有技能共享状态检测、自动推进逻辑和状态协议；预构建文件已提交，用户无需运行构建
...
**superomni 架构（4层）：**
```
Layer 1: Session Hook → 注入 using-skills
...
| **代码审查** | ✅ `requesting-code-review` | ✅ `/review`（Staff Engineer 角色） | ✅ `code-review`（giving/receiving 模式） |
| **跨模型审查** | ❌ | ✅ `/codex`（OpenAI Codex 独立审查） | ❌（roadmap 中） |
| **QA/真实浏览器测试** | ❌ | ✅ `/qa`（Playwright 真实浏览器）+ `/qa-only` | ✅ `qa`（无真实浏览器） |
| **安全审计** | ❌ | ✅ `/cso`（OWASP Top 10 + STRIDE，17个误报排除） | ✅ `code-review`（security 模式）+ `security-auditor`（深审） |
...
| **周回顾** | ❌ | ✅ `/retro`（团队感知 + 全局多项目） | ✅ `self-improvement`（retro scope，git 原语实现） |
...
Agent 在代码审查后 → 进入 code-review 的 receiving 模式
```
...
gstack 的 `/cso`（Chief Security Officer）技能不仅做 OWASP Top 10 + STRIDE，还内置了**17个已知误报排除规则**和 8/10+ 置信度门控。每个安全发现必须包含具体的利用场景（exploit scenario）。这比 superomni 当前的 `code-review`（security 模式）在误报控制上更精细。
...
#### ✅ 自动化 CI 测试套件（v0.6.6 起）
**已修复。** 自 v0.6.6 起，GitHub Actions (`.github/workflows/validate.yml`) 在 ubuntu + windows 两个 job 上跑 9 个 CI gate：

- `npm run gen-skills` — 重建所有 SKILL.md
- `npm run check:skill-docs` — `.tmpl` ↔ `SKILL.md` 一致性 + 4 条 advisory（300+ 行无 reference/、扁平 reference.md、CRLF、token-literal 在 prose）
- `npm run check:workflow-contract` — skill `produces:` ↔ `consumes:` 链路
- `npm run score:workflow` — 流水线工件评分
- `bash lib/validate-skills.sh` — Iron Law / phase 结构 / preamble 展开
- `npm run verify:fixture-parity` — 三个 generator (js/sh/ps1) 的 sha256 一致性
- `npm run test:generators` — multi-occurrence 第一次出现回归测试
- `npm run check:plugin-sync` — 5 个 logical invariant（manifest 版本、commands 集合、keywords 集合、5 个 doc 版本锚点、agent-doc 标题集合）
- `npm run check:plan-content` — destructive 操作前的 careful 步骤 CI 硬门（v0.6.7 起）

每个 PR 提交都自动跑这 9 个 gate；任何一个失败拦截合并。

#### ✅ SKILL.md.tmpl + SKILL.md 双文件已 CI 检查
**已修复。** 自 v0.6.0 起，`lib/check-skill-docs.js` 在 CI 中检测 `.tmpl` 与生成的 `SKILL.md` 是否同步。三个 generator (js/sh/ps1) 通过 golden fixture 验证字节级一致 (sha256)。"忘记重建"会被 CI 立刻拦截。

#### ✅ 版本文档同步 CI 强制（v0.6.5+ 起）
**已修复。** `lib/check-plugin-sync.js` 的 Invariant 4 验证 5 个 `**Last updated:**` 锚点（README、COMPARISON、DESIGN、AGENTS、IMPLEMENTATION）必须等于 `package.json` 版本。Invariant 1 验证 5 个 manifest 版本一致（package.json、marketplace.json ×2、plugin.json、claude-skill.json）。版本漂移在 CI 阶段被拦截。

#### ❌ 缺少自动升级机制（仍待办）
没有等价于 gstack `/gstack-upgrade` 的自我更新命令。用户需要手动重新 `npm install` 才能获取更新。这是 v0.7.0+ 待办项。
...
#### ⚠️ self-improvement 的 retro scope 功能仍有限
gstack 的 `/retro` 有**团队感知**（按人统计提交、shipping streaks、测试健康趋势）和**全局多项目**（`/retro global` 跨越所有项目和 AI 工具）。superomni 当前在 `self-improvement` 的 `retro` scope 里仍以基础 git 日志分析为主，缺乏这些更深入的洞察。
...
#### ✅ 完整 CHANGELOG.md（v0.6.0+ 起维护）
**已修复。** `CHANGELOG.md`（45KB+）从 v0.6.0 起维护，每个 minor 和 patch 都有结构化条目：Fixed / Added / Changed / Removed / Deferred 分类，含每条变更的 rationale 和 verified demos。npm tarball 自 v0.6.5 起也包含 CHANGELOG（`package.json` `files` 字段加入）。
...
superomni:   [触发条件或用户命令] → 强制执行技能（/vibe 命令触发完整流水线）
```
...
*本文档基于 superomni v0.6.10、obra/superpowers 和 garrytan/gstack 的公开信息和代码库分析生成。*
