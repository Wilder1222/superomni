...
| **测试文化** | RED-GREEN-REFACTOR 强制循环，在测试前写的代码必须删除 | 每个 Bug 修复必须生成回归测试；`/ship` 会引导建立测试框架 | TDD 技能存在，但非全局强制 |
...
| **头脑风暴/设计** | ✅ `brainstorming` | ❌ | ✅ `brainstorming` |
...
| **TDD 测试** | ✅ 强制 RED-GREEN-REFACTOR | ✅（每次 bug 修复生成回归测试） | ✅ `test-driven-development`（非强制） |
...
```
Agent 在写代码前 → 强制检查 brainstorming
...
#### ⚠️ TDD 执行力度不够
superomni 有 `test-driven-development` 技能，但与 superpowers 不同：
- 不是全局强制触发的
- 没有"测试前写的代码必须删除"这类强硬规则
- 在 `executing-plans` 中不会主动触发 TDD

**影响：** 实际使用中 TDD 可能被跳过，降低了代码质量保障。

#### ⚠️ 技能流水线缺乏数据传递
gstack 的技能之间有显式的数据流（设计文档传给计划评审，测试计划传给 QA）。superomni 的 `workflow` 技能只是"协调顺序"，没有定义技能之间的输出格式和输入约定，信息传递依赖 AI 的上下文记忆，不够可靠。
...
**P0（核心功能缺口）：**
1. 添加自动化测试套件（至少技能格式验证）
2. 添加 GitHub Actions CI（构建 + 格式验证）
3. 文档版本同步（修复 `docs/DESIGN.md` 版本滞后问题）

**P1（竞争力差距）：**
4. 团队共享机制（`setup --team` 命令，将 superomni 提交到项目仓库）
5. 自动升级机制（`bin/superomni-cli upgrade` 命令）
6. 技能间数据传递约定（定义技能输出格式，供下游技能读取）
7. 添加 CHANGELOG.md

**P2（长期差异化）：**
8. Dual Voices 实现（Claude + Codex 的 plan-review 共识）
9. 基础浏览器测试集成（不需要完整 Playwright，但需要有路径）
10. 社区渠道（Discord 或 GitHub Discussions）
11. 上架 Claude 官方 Marketplace
