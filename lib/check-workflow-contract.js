...
function main() {
    ...
    for (const [flowId, group] of byFlow.entries()) {
        ...
        // REFLECT gate: an evaluation must be followed by EITHER a standalone improvement-*.md
        // OR a release-*.md (which since v0.5.8 contains the `## Retrospective` section per
        // the `self-improvement` skill's "retro merged into release" convention).
        if (types.has("evaluation") && !types.has("improvement") && !types.has("release")) {
            report(`flow ${flowId}: has evaluation but missing improvement or release (REFLECT gate)`);
        }
    ...
    // unless a dir is intentionally retired-but-kept. For a NEW artifact type,
    // add the skill's produces: declaration BEFORE creating the disk dir — the
    // allowlist is NOT an escape hatch for new-but-not-yet-declared dirs.
    ...
    for (const name of subdirs) {
        ...
        // Normalize backslashes to forward slashes so a Windows-authored produces:
        // path (e.g. "docs\superomni\foo\...") still matches the POSIX prefix.
        const covered = allProducePatterns.some(p => p.pattern && p.pattern.replace(/\\/g, "/").startsWith(prefix));
