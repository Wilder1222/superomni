...
if (!fs.existsSync(setupScript)) {
  ...
  process.stderr.write(
    'super-omni postinstall: setup script not found, skipping.\n'
  );
  process.exit(0);
}
...
try {
  ...
  process.stderr.write(
    `super-omni postinstall: setup returned non-zero exit (${err.status}). ` +
      'You can run it manually: super-omni\n'
  );
}
