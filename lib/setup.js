...
function buildSkillDocs() {
    ...
    try {
        // Attempt to run gen-skill-docs.sh if available (optional enhancement)
        const genScript = path.join(SCRIPT_DIR, 'lib', 'gen-skill-docs.sh');
        if (fileExists(genScript)) {
            // Try to run it with explicit bash path resolution
            try {
                const bashPath = process.platform === 'win32' ? 'bash' : 'bash';
                execSync(`${bashPath} "${genScript}"`, {
                    cwd: SCRIPT_DIR,
                    stdio: 'ignore',
                });
                log('✓ Skill docs built via gen-skill-docs.sh');
            } catch (_err) {
                // Silently fail if gen-skill-docs.sh has issues
                // Skills will still work without generated docs
