...
function installCopilot() {
    ...
    if (shouldDo) {
        ...
        // Copilot requires YAML frontmatter with mode and description to recognize commands
        const cmdsDir = path.join(SCRIPT_DIR, 'commands');
        ...
        if (fileExists(cmdsDir) && !DRY_RUN) {
            ...
            for (const cmdFile of cmdFiles) {
                ...
                const firstLine = mdContent.split('\n').find(l => l.trim())
                    || cmdName;
                const description = firstLine.replace(/^#+\s*/, '').replace(/^\/\S+\s*/, '').trim()
                    || `superomni ${cmdName} skill`;
