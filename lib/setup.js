...
function getAgentNames() {
    const dir = path.join(SCRIPT_DIR, 'agents');
...
function installClaude() {
    ...
    if (TARGET_DIR) {
        ...
        const content = claudeInstructions.generate('.superomni/skills', getCommandNames(), getAgentNames());
        ...
        // Install agents into <project>/.claude/agents/
        // Claude Code scans this path for project-level sub-agents
        const agentsDir = path.join(SCRIPT_DIR, 'agents');
        const projectAgentsDir = path.join(TARGET_DIR, '.claude', 'agents');
        if (fileExists(agentsDir) && !DRY_RUN) {
            ensureDir(projectAgentsDir);
            const agentFiles = fs.readdirSync(agentsDir).filter(f => f.endsWith('.md'));
            for (const agentFile of agentFiles) {
                const src = path.join(agentsDir, agentFile);
                const dest = path.join(projectAgentsDir, agentFile);
                if (!FORCE && fileExists(dest)) continue;
                try {
                    fs.copyFileSync(src, dest);
                } catch (_e) { /* skip on error */ }
            }
            log(`✓ Claude Code agents installed (${agentFiles.length} agents) → ${projectAgentsDir}`);
        }
    } else {
        ...
        // Claude Code scans this path for personal sub-agents
        const agentsDir = path.join(SCRIPT_DIR, 'agents');
        ...
        if (fileExists(agentsDir)) {
            ...
            for (const agentFile of agentFiles) {
                symlinkFile(
                    ...
                    path.join(claudeAgentsDir, agentFile)
                );
            }
            if (!DRY_RUN) {
                log(`✓ Claude Code agents linked (${agentFiles.length} agents) → ${claudeAgentsDir}`);
            }
        }
    }
}
...
function installCodex() {
    ...
    if (TARGET_DIR) {
        ...
        // Note: Codex CLI does NOT support custom slash commands via project files.
        // All slash commands (/Model, /Compact, etc.) are hardcoded in the binary.
        // Skills are loaded via .codex/skills/<name>/SKILL.md and accessible through /Skills picker.
        log('ℹ Codex CLI: trigger skills via AGENTS.md context (no custom slash commands supported)');
    } else {
...
function installCopilot() {
    ...
    if (shouldDo) {
        ...
        // Copilot discovers these as slash commands via the 'agent' frontmatter field
        const cmdsDir = path.join(SCRIPT_DIR, 'commands');
        ...
        if (fileExists(cmdsDir) && !DRY_RUN) {
            ...
            for (const cmdFile of cmdFiles) {
                const cmdName = cmdFile.replace(/\.md$/, '');
                const mdContent = readFile(path.join(cmdsDir, cmdFile));
                // Find first non-heading, non-empty line for description
                const lines = mdContent.split('\n');
                const descLine = lines.find(l => l.trim() && !l.startsWith('#')) || cmdName;
                const description = descLine.trim().replace(/'/g, "\\'").slice(0, 80);
                // 'agent' is the correct frontmatter field name (not 'mode')
                const promptContent = `---\nagent: 'agent'\ndescription: '${description}'\n---\n${mdContent}`;
                try {
                    writeFile(path.join(promptsDir, cmdName + '.prompt.md'), promptContent);
                } catch (_e) { /* skip on error */ }
            }
            log(`✓ GitHub Copilot prompts installed (${cmdFiles.length} commands) → ${promptsDir}`);
            log('ℹ Copilot slash commands work in VS Code extension only. For CLI use, install via: /plugin marketplace add Wilder1222/superomni');
        }
    } else {
...
function registerHooks() {
    ...
    if (!DRY_RUN && fileExists(hooksSource)) {
        ...
        const sessionStartWrapper = path.join(claudeHooksDir, 'superomni-session-start.sh');
        const sessionStartScript = path.join(SCRIPT_DIR, 'hooks', 'session-start').replace(/\\/g, '/');
        const wrapperContent = `#!/usr/bin/env bash\nexec "${sessionStartScript}" "$@"\n`;
        writeFile(sessionStartWrapper, wrapperContent);
        writeFile(path.join(claudeHooksDir, 'superomni-hooks.json'), readFile(hooksSource));
...
function initializeConfig() {
    ...
    if (!DRY_RUN && !fileExists(configFile)) {
        ...
        writeFile(configFile, 'telemetry=true\n');
        log('✓ Default config initialized (telemetry=true)');
    } else if (DRY_RUN) {
