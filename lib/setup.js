...
function buildSkillDocs() {
    ...
    try {
        ...
        if (!fileExists(preambleFile)) {
            log('⚠ preamble.md not found (optional)', 'WARN');
            return;
        }
        ...
        // Find and process all SKILL.md.tmpl files
        const walkSkills = (dir) => {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        const tmplContent = readFile(fullPath);
                        // Replace {{PREAMBLE}} (only first occurrence)
                        const skillContent = tmplContent.replace(
                            /\{\{PREAMBLE\}\}/,
                            preambleContent
                        );
                        const skillFile = fullPath.replace(/\.tmpl$/, '');
                        writeFile(skillFile, skillContent);
                        VERBOSE && log(`✓ ${fullPath} → ${skillFile}`);
                    } catch (err) {
                        log(`⚠ Failed to process ${fullPath}: ${err.message}`, 'WARN');
                    }
                }
            }
        };
