...
- If approved (e.g., user replies "继续" / "approve" / "A") → report DONE and auto-advance to PLAN (`writing-plans`). All subsequent stages (PLAN → REVIEW → BUILD → VERIFY → RELEASE) execute automatically without further user input.
