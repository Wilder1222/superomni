...
- If the template file is unavailable, fallback to sections in `reference/design-md-adaptation.md` (Positioning, Quality Gate Mapping, Anti-Copy Guardrails), keep output file names as `design-md-<brand>.md` and `prompt-design-md-<brand>.md`, and structure output into these blocks: `Metadata`, `Transferable Rules`, `Quality Gate Mapping`, `Anti-Copy Constraints`, `Prompt Fragment`.
