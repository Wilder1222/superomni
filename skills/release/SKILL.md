...
Confirm:
- [ ] On correct branch; working tree is clean before entering RELEASE
...
## Phase 5: Finalize Release Changes (Conditional Auto-Commit)

Run this only after release actions and `document-release` updates are complete.

```bash
# Commit only if release workflow produced changes
if [ -n "$(git status --porcelain)" ]; then
  git add -A
  git commit -m "release: finalize release artifacts and docs"
  RELEASE_COMMIT=$(git rev-parse --short HEAD 2>/dev/null || echo "")
  RELEASE_COMMIT_STATUS="committed"
else
  RELEASE_COMMIT=""
  RELEASE_COMMIT_STATUS="skipped"
fi
```

Rules:
- If commit succeeds -> continue.
- If no changes are present -> skip commit and record the skip reason.
- If commit is required but fails -> report `BLOCKED` (do not report `DONE`).
...
## Release:   [populated ✓]
## Retro:     [populated ✓]
Commit:       [committed: <hash> | skipped: no changes]
