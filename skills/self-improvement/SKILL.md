...
when_to_use: |
  Use for process retrospection (three scopes: process / retro / harness). NOT for checking whether a specific task is done — use verification for that.
...
> **Deprecated in v0.5.8.** Retrospective data is now written inside the release artifact
> (`docs/superomni/releases/release-*.md`, `## Retrospective` section) by the `release` skill.
> When running in `retro` scope, write retro content into the release artifact if it exists,
> or into `docs/superomni/improvements/improvement-[branch]-[session]-[date].md` as a section.
> Do NOT create standalone `docs/superomni/retros/` files.
...
If any gaps are found, generate `docs/superomni/improvements/tacit-gaps-[date].md`. Use the template in [reference/phase-templates.md § Phase 0](${CLAUDE_SKILL_DIR}/reference/phase-templates.md#phase-0--tacit-gap-output-template).
...
|----------|--------|----------|
| Did each major task follow the THINK→PLAN→REVIEW→BUILD→VERIFY→RELEASE cycle? | | |
...
Evaluate the AI agent's performance on three 1-5 dimensions: Scope Management, Instruction Following, Escalation Behavior. Use the scoring rubrics in [reference/phase-templates.md § Phase 3](${CLAUDE_SKILL_DIR}/reference/phase-templates.md#phase-3--agent-behavior-scoring-rubrics) and record `Score __/5 — Evidence: ___` for each. Total: **__ / 15**.
...
Generate exactly **3 concrete improvement actions** for the next sprint, using the format and worked example in [reference/phase-templates.md § Phase 6](${CLAUDE_SKILL_DIR}/reference/phase-templates.md#phase-6--improvement-action-template).
...
Save the **full** evaluation report to `$REPORT_FILE`. All scores and tables from Phases 1–6 must be included — not just the action items. Use the canonical structure in [reference/phase-templates.md § Phase 7](${CLAUDE_SKILL_DIR}/reference/phase-templates.md#phase-7--full-improvement-report-template).
...
End the session with the `SELF-IMPROVEMENT REPORT` block defined in [reference/phase-templates.md § Final SELF-IMPROVEMENT REPORT Block](${CLAUDE_SKILL_DIR}/reference/phase-templates.md#final-self-improvement-report-block).