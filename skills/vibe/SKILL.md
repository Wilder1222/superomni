...
RELEASE expectation: after `release` and `document-release` complete, run a conditional auto-commit for release changes (commit when dirty, skip when clean, non-DONE on commit failure).
