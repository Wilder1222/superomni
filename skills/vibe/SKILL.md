...
Scan for existing artifacts to determine where the project is in the sprint pipeline. The full session-aware detection script + artifact-verification helper is in [reference/stage-detection.md](${CLAUDE_SKILL_DIR}/reference/stage-detection.md). Run it, then proceed with the matrix below.
