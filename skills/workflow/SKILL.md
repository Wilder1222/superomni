...
name: workflow
description: |
  Reference document describing the 6-stage pipeline and skill data flow. Triggers: "workflow", "pipeline". NOT an action — use vibe to run the pipeline.
allowed-tools: [Bash, Read, Write, Edit, Grep, Glob]
when_to_use: |
  Use as a lookup when you need to know which skill runs at which stage or what artifact each stage produces. Does not produce artifacts itself.
produces: ~
consumes: ~
---
...
Artifacts: spec-*.md [Y/N] | plan-*.md [Y/N] | executions [N] | reviews [N] | prod-readiness [N] | releases [N]
