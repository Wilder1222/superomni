...
> **CI-enforced since v0.6.7** via `npm run check:plan-content` (see [`lib/check-plan-content.js`](${CLAUDE_SKILL_DIR}/../../lib/check-plan-content.js)). Plans that violate this gate fail CI before reaching execution. The linter scans each plan's `**How:**` subsections, ignoring fenced code blocks, and verifies the immediately-preceding step (in document order) contains the keyword `careful`.
